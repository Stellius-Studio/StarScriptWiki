## Description
Reads a MongoDB-backed variable as a number, adds an amount (positive or negative), and writes
the result back. A missing variable is created as `FLOAT` starting from `0`. Fails
...
AddRedScore:
  Skills:
  - addmongo{var=score_red;a=1} @trigger
```
...
AddPartyQuestProgress:
  Skills:
  - addmongo{var=quest_progress;a=1;scope=custom;group=<skill.var.party_id>} @trigger
```
...
## See Also
- [setmongo](Skills-Mechanics-setmongo) — write a value with an explicit type.
- [addsql](Skills-Mechanics-addsql) / [addredis](Skills-Mechanics-addredis) — same shape, SQL/Redis-backed.
